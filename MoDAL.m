classdef MoDAL
    ...
    methods(Static)
        ...
        function T = TRAC(signal1,signal2)
            % Computes the TRAC metric for two time series.
            %
            % Required Inputs
            % ---------------------------------------------
            ...
            % Output
            % ---------------------------------------------
            % T - Scalar value providing a measure of similarity between two signals
            T = (signal1'*signal2)^2/((signal1'*signal1)*(signal2'*signal2));
        end

        function R = RSquared(signal1,signal2)
            % Computes the R-squared metric for two time series.
            %
            % Required Inputs
            % ---------------------------------------------
            % signal1 - Truth signal that signal2 is compared with
            % signal2 – Second signal for comparison
            %
            % Output
            % ---------------------------------------------
            % R - R-squared value
            SS1 = sum((signal1-mean(signal1)).^2);
            SS2 = sum((signal1-signal2).^2);
            R = 1-SS2/SS1;
        end

        function M = MSE(signal1,signal2)
            % Computes the mean squared error for two time series.
            %
            % Required Inputs
            % ---------------------------------------------
            % signal1 - Truth signal that signal2 is compared with
            ...
            % ---------------------------------------------
            % R - R-squared value
            M = mean((signal1-signal2).^2);
        end

        function M = MAE(signal1,signal2)
            % Computes the mean absolute error for two time series.
            %
            % Required Inputs
            % ---------------------------------------------
            % signal1 - Truth signal that signal2 is compared with
        ...
        function [natFreq,Phi] = ComputeNormalModes(stiffnessMatrix,massMatrix,radians)
            ...
            % Required Inputs
            % ---------------------------------------------
            % stiffnessMatrix - The stiffness matrix for the system.
        ...
        function Phi = MassOrthonormalize(massMatrix,modeShapes)
            ...
            % ---------------------------------------------
            % massMatrix - The mass matrix for the system.
            % modeShapes - The modal matrix where each column represents a single mode.
            %
            % Outputs
            % ---------------------------------------------
            % Phi - Modal matrix where each column represents a mass-orthonormalized mode shape.
            Phi = 0*modeShapes;
            for i = 1:length(massMatrix)
                Phi(:,i) = modeShapes(:,i)/sqrt(modeShapes(:,i)'*M*modeShapes(:,i));
            end
        end
        ...
        function PlotTSWT_Compare(time1,signal1,time2,signal2,minFreq,maxFreq,options)
            ...
            if tA1 == 0; tA1 = 1; end
            if tA2 == 0; tA2 = 1; end
            if tB1 == 0; tB1 = length(time1); end
            if tB2 == 0; tB2 = length(time2); end
            if options.radians
        ...
        function IMF = WBEMD(time,signal,minFreq,maxFreq,charFreq,options)
            ...
            % Advanced Inputs
            % ---------------------------------------------
            % Mirroring is an advanced technique for minimizing end effects in the extracted IMFs.
            ...
            %
            %       Mirror.Ini - A cell that contains whether the even or odd mirroring should be
            %                    applied to the beginning of the signal. Each entry in the cell must
            %                    be a vector of 0s and 1s, where a value of 0 corresponds to odd
            ...
            % maxFreq = 20;
            % charFreq{1} = [15;10];
            % Mirror.On = 0; % Turn off mirroring
            ...
            % Copyright
            % ---------------------------------------------
            % Copyright (c) Keegan J. Moore, 2025
            % kmoore@gatech.edu
            %
            % Citations
            % ---------------------------------------------
            ...
            for b = 1:M
                ...
                if options.Mirror.On == 1
                    ...
                    mirrorFin = options.Mirror.Ini{b};
                end
                ...
                while(1)
                    ...
                    if options.Mirror.On == 1
                        if fg(o) == fmirr(m)
                            ...
                            [x_o_ini,ti_o_ini] = MoDAL.MirrorImgSigOIni(ti(1:L_chp1),X_emd(1:L_chp1));
                            [x_e_ini,ti_e_ini] = MoDAL.MirrorImgSigEIni(ti(1:L_chp1),X_emd(1:L_chp1));
                            [x_o_fin,ti_o_fin] = MoDAL.MirrorImgSigOFin(ti(end-L_chp2+1:end),X_emd(end-L_chp2+1:end));
                            [x_e_fin,ti_e_fin] = MoDAL.MirrorImgSigEFin(ti(end-L_chp2+1:end),X_emd(end-L_chp2+1:end));
                            if mirrorIni(m) == 0
                                if mirrorFin(m) == 0
                                    X_emd_mirror = [x_o_ini; X_emd; x_o_fin];
                                    ti_mirror = [ti_o_ini; ti ; ti_o_fin];
                                else
                                    X_emd_mirror = [x_o_ini; X_emd; x_e_fin];
                                    ti_mirror = [ti_o_ini; ti ; ti_e_fin];
                                end
                            else
                                if mirrorFin(m) == 0
                                    X_emd_mirror = [x_e_ini; X_emd; x_o_fin];
                                    ti_mirror = [ti_e_ini; ti ; ti_o_fin];
                                else
                                    X_emd_mirror = [x_e_ini; X_emd; x_e_fin];
                                    ti_mirror = [ti_e_ini; ti ; ti_e_fin];
                                end
                            end
                            X_emd = X_emd_mirror;
                            ti = ti_mirror;
                    ...
                    if options.Mirror.On == 1
                        if fg(o,1) == fmirr(m)
                            IMF_Proc = IMF_Proc(:,L_chp1:end-L_chp2+1);
                        end
                    end
